...
class AdminEditProfile extends ConsumerStatefulWidget {
  final User user;

  const AdminEditProfile({super.key, required this.user});

  @override
  ConsumerState<AdminEditProfile> createState() => _AdminEditProfileState();
}

class _AdminEditProfileState extends ConsumerState<AdminEditProfile> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: GestureDetector(
          onTap: () {
            Navigator.push(
                context, toRightTransition(const AdminAccountDetails()));
          },
          child: const Padding(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildInputField('Staff ID', _lecturerIdController, onTap: () {
            QuickAlert.show(
              context: context,
              type: QuickAlertType.info,
              title: 'Info',
              text: 'Staff ID cannot be changed.',
              confirmBtnText: 'OK',
            );
          }),
          const SizedBox(height: 16),
                ...
                onConfirmBtnTap: () async {
                  if (user.userId != null) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content:
                            Text('Staff ID is null. Cannot update profile.'),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
